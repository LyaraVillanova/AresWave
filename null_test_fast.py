...
# -*- coding: utf-8 -*-
"""
Null test = benchmark for PSO, using the SAME objective as areswave.PSO.create_cost_function.

Important:
- The misfit / preprocessing path is unchanged (same as PSO objective).
- Optional speed-up: build 6 DSM basis synthetics (Mrr,Mtt,Mpp,Mrt,Mrp,Mtp) once per (event, model, depth)
  and reuse them via linear combination for each random mechanism. This does NOT change results; it only avoids
  rerunning DSM hundreds of times (linearity of synthetics in the moment tensor).

What this script does:
- For each (event, model) in BEST_SOLUTIONS:
    - fixes depth = best-fit depth
    - trims real data to [P-5s, S+10s] so indices are always in-range (PSO misfit uses P±5 and S-5..+10)
    - draws N random (strike, dip, rake) inside the SAME bounds used in your PSO scripts
      (strike [0,360], dip [0,90], rake [-180,180])
    - evaluates the objective on those draws (batched)
...
# Speed-up for null tests only (does NOT touch your PSO code):
RUN_USE_DSM_BASIS_CACHE = True     # build 6 MT basis synthetics once per (event,model,depth) and reuse
RUN_BASIS_CACHE_DIR = "null_test_cache_dsm"  # directory name under current working dir
...
BEST_SOLUTIONS = {'S0133a': {'TAYAK': {'depth_km': 24.7, 'strike_deg': 301.75, 'dip_deg': 34.76, 'rake_deg': 55.95, 'cost': 0.0407},
 ...
 'S0226b': {'TAYAK': {'depth_km': 11.4815412094259,
            ...
            'X': {'depth_km': 11.69686465,
                      'strike_deg': 300.77483763,
                      'dip_deg': 83.18932457,
                      'rake_deg': -78.58439998,
                      'cost': 0.0965}},
 ...
 'S0421b': {'TAYAK': {'depth_km': 31.29616013,
            ...
            'X': {'depth_km': 30.27869612,
                  ...
                  'cost': 0.0738}},
 'S0976a': {'TAYAK': {'depth_km': 68.4200934746984,
                      'strike_deg': 234.999014181556,
                      'dip_deg': 9.60575960694102,
                      'rake_deg': 57.9338382647017,
                      'cost': 0.146384626376218},
            'X': {'depth_km': 42.0440773235149,
                  'strike_deg': 272.360733224122,
                  'dip_deg': 32.9514779568056,
                  'rake_deg': 65.7025104395448,
                  'cost': 0.14647802118839}},
...
# =============================================================================
# DSM basis cache for fast null tests (no changes to PSO code)
# =============================================================================

def _basis_cache_key(ev_id: str, model_key: str, depth_km: float, tlen: float, nspc: int, hz: float) -> str:
    s = f"{ev_id}|{model_key}|depth={depth_km:.5f}|tlen={tlen:.3f}|nspc={int(nspc)}|hz={float(hz):.3f}"
    return hashlib.sha1(s.encode("utf-8")).hexdigest()

def _load_basis_npz(path: Path):
    ...
    return {
        ...
        "R": d["R"],
        "T": d["T"],
        "order": [x for x in d["order"].tolist()],
    }

def _save_basis_npz(path: Path, ts: np.ndarray, Z: np.ndarray, R: np.ndarray, T: np.ndarray, order: list):
    path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(path, ts=ts, Z=Z, R=R, T=T, order=np.array(order, dtype=object))

def _build_or_load_basis(ev_id: str, model_key: str, base_event: Event, stations, seismic_model,
                        tlen: float, nspc: int, hz: float, cache_dir: Path, quiet: bool):
    # Order matches MomentTensor ctor used in PSO: (Mrr, Mrt, Mrp, Mtt, Mtp, Mpp)
    order = ["Mrr", "Mrt", "Mrp", "Mtt", "Mtp", "Mpp"]
    key = _basis_cache_key(ev_id, model_key, float(base_event.depth), float(tlen), int(nspc), float(hz))
    path = cache_dir / f"basis_{key}.npz"

    if path.exists():
        try:
            b = _load_basis_npz(path)
            # Basic sanity
            if b.get("Z", np.empty((0,))).ndim == 2 and b["Z"].shape[0] == 6 and b.get("order") == order:
                print(f"[cache] Loaded DSM basis: {path}", flush=True)
                return b
        except Exception:
            pass

    print(f"[cache] Building DSM basis (6 runs, full MT) for {ev_id}/{MODEL_NAME.get(model_key, model_key)} depth={float(base_event.depth):.2f} km", flush=True)
    Z_list, R_list, T_list = [], [], []
    ts_ref = None

    # Build one unit basis per independent component
    for k, name in enumerate(order, start=1):
        coeffs = {nm: 0.0 for nm in order}
        coeffs[name] = 1.0
        mt = MomentTensor(coeffs["Mrr"], coeffs["Mrt"], coeffs["Mrp"], coeffs["Mtt"], coeffs["Mtp"], coeffs["Mpp"])
        ev = Event(
            event_id=f"{ev_id}__{name}",
            latitude=float(base_event.latitude),
            longitude=float(base_event.longitude),
            depth=float(base_event.depth),
            mt=mt,
            centroid_time=base_event.centroid_time,
            source_time_function=None,
        )
        t0 = time.time()
        with _maybe_quiet(quiet):
            out = PSO.generate_synthetics(ev, stations, seismic_model, float(tlen), int(nspc), float(hz))
        # Use the same extraction pattern as PSO
        ts = out.ts
        max_idx = int(np.searchsorted(ts, float(tlen)))
        uZ = out["Z", "ELYSE_XB"][:max_idx]
        uR = out["R", "ELYSE_XB"][:max_idx]
        uT = out["T", "ELYSE_XB"][:max_idx]
        if ts_ref is None:
            ts_ref = ts[:max_idx]
        Z_list.append(np.asarray(uZ, dtype=float))
        R_list.append(np.asarray(uR, dtype=float))
        T_list.append(np.asarray(uT, dtype=float))
        print(f"[cache]   {k}/6 basis {name} done in {time.time()-t0:.1f}s", flush=True)

    Z = np.stack(Z_list, axis=0)
    R = np.stack(R_list, axis=0)
    T = np.stack(T_list, axis=0)
    _save_basis_npz(path, ts_ref, Z, R, T, order)
    print(f"[cache] Saved DSM basis: {path}", flush=True)
    return {"ts": ts_ref, "Z": Z, "R": R, "T": T, "order": order}

def _create_cost_function_basis_cached(ev_id: str, model_key: str, base_event: Event, stations, seismic_model,
                                      tlen: float, nspc: int, sampling_hz: float, real_data_list,
                                      magnitude: float, distance: float, time_p: UTCDateTime, time_s: UTCDateTime,
                                      cache_dir: Path, quiet: bool):
    # Mirror PSO.create_cost_function() as closely as possible, but replace DSM compute with basis linear combination.
    real_data_list = PSO.reorder_traces(real_data_list)
    max_shift_samples = int(2.0 * float(sampling_hz))

    basis = _build_or_load_basis(ev_id, model_key, base_event, stations, seismic_model, float(tlen), int(nspc), float(sampling_hz), cache_dir, quiet)
    ts = basis["ts"]
    max_idx = int(np.searchsorted(ts, float(tlen)))
    Zb = basis["Z"][:, :max_idx]
    Rb = basis["R"][:, :max_idx]
    ...
    def cost_func(x):
        ...
        for i, (dpt, stke, dp, rk) in enumerate(x):
            # Keep same depth validity gate as PSO
            if dpt < 10 or dpt > 560:
                costs.append(1e6)
                continue

            try:
                mts = PSO.calculate_moment_tensor(float(magnitude), float(stke), float(dp), float(rk), float(dpt), float(distance),
                ...
                print(f"Error to calculate the moment tensor: {e}", flush=True)
                costs.append(1e6)
                continue

            # Combine basis (order: Mrr, Mrt, Mrp, Mtt, Mtp, Mpp)
            coeff = np.array([Mrr, Mrt, Mrp, Mtt, Mtp, Mpp], dtype=float)
            u_Z = coeff @ Zb
            u_R = coeff @ Rb
            u_T = coeff @ Tb
            ...
            try:
                ...
                syn_times = np.arange(n_samples) / float(sampling_hz)
                ...
                print(f"Erro na calculate_variation: {e}", flush=True)
                var = 1e6
...
def run_one(ev_id: str, model_key: str, sac_dir: Path, out_dir: Path, rng: np.random.Generator,
    ...
    if RUN_USE_DSM_BASIS_CACHE:
        cache_dir = Path.cwd() / str(RUN_BASIS_CACHE_DIR)
        cost_func = _create_cost_function_basis_cached(
            ev_id, model_key, event, stations, seismic_model,
            float(tlen), int(NSPC), float(SAMPLING_HZ), real_trim,
            float(mag), float(dist), time_p, time_s,
