...
from dsmpy.event_Mars import Event
...
class PyDSMOutput:
    ...
    def get_us_shape(self) -> Tuple[int, int, int]:
...
class DSMInput:
    ...
    def input_from_arrays(cls, event, stations,
        ...
        maxnr = tish_parameters['maxnr']
        output = np.full((maxnr,), b' ' * 80, dtype='S80')  # 1-D array of 80-byte strings

        for i, station in enumerate(stations):
            string = f"{station.name}_{station.network}.{event.event_id}SH.spc"
            # garante 80 bytes e converte para bytes
            sbytes = string.ljust(80)[:80].encode('ascii')
            output[i] = sbytes  # dtype='S80' aceita bytes
    ...
    def get_inputs_for_tish(self):
        ...
        inputs = (self.re, self.ratc, self.ratl, self.tlen,
                ...
                self.theta, self.phi, self.lat, self.lon, output_flat)
        return inputs
...
class PyDSMInput(DSMInput):
    ...
    def _parse_stations(self):
        """Parse station name/network a partir de self.output (array S1)."""
        stations = []
        for i in range(self.nr):
            raw = self.output[i].tobytes().decode('utf-8').strip()
            parts = raw.split('/')[-1].split('.')[0].split('_')
            if len(parts) == 2:
                name, net = parts
            else:
                name = parts[0]
    ...
    def _parse_event(self):
        raw = self.output[0].tobytes().decode('utf-8').strip()
        print("DEBUG raw event string:", raw)
        parts = raw.split('/')[-1].split('.')
        
        # Se não tem o formato esperado, aceite o que tiver sem erro:
        if len(parts) > 1:
            event_id = parts[1]
        else:
            event_id = parts[0]  # usa o que tiver, mesmo que seja só um nome simples

        # Remover sufixos SH ou PSV se existirem, mas não exigir
        if event_id.endswith('SH'):
            event_id = event_id[:-2]
        elif event_id.endswith('PSV'):
            event_id = event_id[:-3]

        # Agora crie o objeto Event normalmente
        event = Event(
            event_id,
            self.eqlat, self.eqlon,
            3389.5 - self.r0,
            self.mt,
            None, None
        )
