...
from areswave.PSOearth2 import create_cost_function, run_pso_with_restarts, plot_results

# Configuration: Event and Station
event_id = 'IU_MAJO'
latitude, longitude, distance, depth = 18.512, 168.062, 290, 31
magnitude = 5.17
...
stations = [Station(name='MAJO', network='IU', latitude=41.9486, longitude=142.5864)]
...
sac_files = [
    os.path.join(sac_folder_path, 'IU_MAJO_00_BHR.sac'),
    os.path.join(sac_folder_path, 'IU_MAJO_00_BHT.sac'),
    os.path.join(sac_folder_path, 'IU_MAJO_00_BHZ.sac')
]
...
for sac_file in sac_files:
    ...
    tr.trim(starttime=time_p - 10, endtime=time_p + 450)
    stream += tr
