...
from areswave.synthetics_function import generate_synthetics, calculate_moment_tensor, calculate_variation, apply_filter, normalize, align_by_correlation
...
def plot_results(best_pos, event, stations, seismic_model, tlen, nspc, sampling_hz,
    ...
    for i, comp in enumerate(("Z", "R", "T")):
        ...
        axs[i, 0].plot(times, syn_data, label="Sintético", color="blue", lw=1, linestyle="-")
        ...
        axs[i, 1].plot(times, syn_data, label="Sintético", color="blue", lw=1, linestyle="-")
        axs[i, 1].axvline(t_s, color="black", linestyle=":", lw=1.5, label="S-wave")
        axs[i, 1].axvspan(t_s - window_size / 2, t_s + window_size, color="lightgray", alpha=0.2)
        axs[i, 1].set_xlim([350, 375])
