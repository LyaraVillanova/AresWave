...
DATA_DIR = "/home/lyara/areswave/SAC17/"
CSV_FILE = "data/arrivals.csv"
...
def load_observed_waveforms(event_id):
    ...
    event_token = "".join(event_id.split())

    base_event_path = os.path.join(DATA_DIR, event_id)
    search_dir = base_event_path if os.path.exists(base_event_path) else DATA_DIR
    ...
    for fname in os.listdir(search_dir):
        ...
        fname_norm = "".join(fname.split())

        if fname_norm.startswith(event_token + "_") or fname_norm.startswith(event_token + "."):
            matched.append(os.path.join(search_dir, fname))

    for fpath in sorted(matched):
        st += read(fpath)

    if len(st) == 0:
        logger.warning(f"[{event_id}] No data found in {search_dir}")
        return None
    st.detrend("demean")
    st.filter("bandpass", freqmin=0.3, freqmax=0.9, zerophase=True)
    logger.info(f"[{event_id}] {len(st)} traces loaded from {search_dir}")
    return st
...
def misfit_vs_depth_heatmap(events, depth_min=700.0, depth_max=1800.0):
    ...
    out = os.path.join(FIG_DIR, "misfit_heatmap_17.png")
...
def plot_layered_fig3_heatmaps(
    ...
    outA = os.path.join(fig_dir, "vp_layered_fig3A_17.png")
    plt.tight_layout()
    ...
    outB = os.path.join(fig_dir, "vs_layered_fig3B_17.png")
    plt.tight_layout()
...
def plot_pnas_style_transition(
    ...
    plt.tight_layout()
    out = os.path.join(output_dir, "vp_vs_pnas_style_transition_17.png")
...
def _evaluate_layer_grid(events, dmin, dmax, vp_values, vs_values):
    n_vp = len(vp_values)
    n_vs = len(vs_values)
    misfit_layer_total = np.zeros((n_vp, n_vs), dtype=float)
    misfit_layer_vp = np.zeros((n_vp, n_vs), dtype=float)
    misfit_layer_vs = np.zeros((n_vp, n_vs), dtype=float)

    for i_vp, vp in enumerate(vp_values):
        for i_vs, vs in enumerate(vs_values):
            m_tot, m_vp, m_vs = compute_layer_misfit(events, float(vp), float(vs), float(dmin), float(dmax))
            misfit_layer_total[i_vp, i_vs] = m_tot
            misfit_layer_vp[i_vp, i_vs] = m_vp
            misfit_layer_vs[i_vp, i_vs] = m_vs

    return misfit_layer_total, misfit_layer_vp, misfit_layer_vs


def _resample_layer_grid(src_vp, src_vs, src_grid, dst_vp, dst_vs):
    interp = RegularGridInterpolator(
        (np.asarray(src_vp, dtype=float), np.asarray(src_vs, dtype=float)),
        np.asarray(src_grid, dtype=float),
        bounds_error=False,
        fill_value=None,
    )
    VP, VS = np.meshgrid(np.asarray(dst_vp, dtype=float), np.asarray(dst_vs, dtype=float), indexing="ij")
    pts = np.column_stack([VP.ravel(), VS.ravel()])
    out = interp(pts).reshape(len(dst_vp), len(dst_vs))
    return np.asarray(out, dtype=float)


def _snap_depths_to_grid(candidate_depths, full_depth_windows):
    full_depth_windows = np.asarray(full_depth_windows, dtype=float)
    if full_depth_windows.size == 0:
        return np.asarray([], dtype=float)

    snapped = []
    lo = float(full_depth_windows.min())
    hi = float(full_depth_windows.max())
    for d in np.asarray(candidate_depths, dtype=float):
        if not np.isfinite(d):
            continue
        if d < lo or d > hi:
            continue
        idx = int(np.argmin(np.abs(full_depth_windows - d)))
        snapped.append(float(full_depth_windows[idx]))
...
def _depth_refine_candidates(center_depths, local_step_km, full_depth_windows, half_width=3):
    cand = []
    for d0 in np.asarray(center_depths, dtype=float):
        for k in range(-int(half_width), int(half_width) + 1):
            cand.append(float(d0 + k * float(local_step_km)))
    return _snap_depths_to_grid(cand, full_depth_windows)


def _interpolate_cube_along_depth(sample_depths, sample_cube, full_depth_windows):
    sample_depths = np.asarray(sample_depths, dtype=float)
    full_depth_windows = np.asarray(full_depth_windows, dtype=float)
    sample_cube = np.asarray(sample_cube, dtype=float)

    if sample_cube.ndim != 3:
        raise ValueError("sample_cube deve ter shape (n_depth, n_vp, n_vs)")
    if sample_depths.size != sample_cube.shape[0]:
        raise ValueError("sample_depths incompatível com sample_cube")

    order = np.argsort(sample_depths)
    z = sample_depths[order]
    cube = sample_cube[order, :, :]

    out = np.zeros((len(full_depth_windows), cube.shape[1], cube.shape[2]), dtype=float)
    ...
    for i in range(cube.shape[1]):
        for j in range(cube.shape[2]):
            out[:, i, j] = np.interp(
                ...
                z,
                cube[:, i, j],
                left=float(cube[0, i, j]),
                right=float(cube[-1, i, j]),
            )
    return out
...
def fit_discontinuities_from_cube(
    ...
    def objective(p):
        ...
        for i, interp in enumerate(interps):
            cost += float(np.asarray(interp(pts[i])).ravel()[0])
        return cost + pen
...
def plot_transition_with_discontinuities(
    ...
    output_dir,
    filename="vp_vs_pnas_style_transition_17.png",
):
    import os
    import numpy as np
    import matplotlib.pyplot as plt
...
# ---------------------------------------------------------------------------
def main():
    logger.info("=== Iniciando inversão camada a camada (busca em três etapas) ===")
    ...
    full_depth_windows = np.arange(float(DEPTH_START_KM), depth_end_eff, step)
    depths_center = full_depth_windows + 0.5 * step
    ...
    #   Etapa 1: step = 60 km, 7x7, em toda a faixa
    #   Etapa 2: step = 40 km, 7x7, refinando os 5 melhores da etapa 1
    #   Etapa 3: step = 20 km, 7x7, refinando os 3 melhores da etapa 2
    n_vp_eval = 7
    n_vs_eval = 7
    vp_values_eval = np.linspace(vp_values.min(), vp_values.max(), n_vp_eval)
    vs_values_eval = np.linspace(vs_values.min(), vs_values.max(), n_vs_eval)

    stage1_depths = _snap_depths_to_grid(
        np.arange(float(DEPTH_START_KM), depth_end_eff, 60.0),
        full_depth_windows,
    )

    if full_depth_windows.size and float(full_depth_windows[-1]) not in stage1_depths:
        stage1_depths = np.asarray(sorted(set(stage1_depths.tolist() + [float(full_depth_windows[-1])])), dtype=float)

    stage_results = {}

    def _evaluate_and_store(depth_list, stage_label):
        stage_scores = {}
        for dmin in np.asarray(depth_list, dtype=float):
            dmin = float(dmin)
            dmax = dmin + step
            logger.info(f"--- {stage_label} | camada {dmin:.0f}-{dmax:.0f} km ---")

            m_tot_e, m_vp_e, m_vs_e = _evaluate_layer_grid(events, dmin, dmax, vp_values_eval, vs_values_eval)

            misfit_layer_total = _resample_layer_grid(vp_values_eval, vs_values_eval, m_tot_e, vp_values, vs_values)
            misfit_layer_vp = _resample_layer_grid(vp_values_eval, vs_values_eval, m_vp_e, vp_values, vs_values)
            misfit_layer_vs = _resample_layer_grid(vp_values_eval, vs_values_eval, m_vs_e, vp_values, vs_values)

            misfit_grid_vp_local = np.min(misfit_layer_vp, axis=1)
            misfit_grid_vs_local = np.min(misfit_layer_vs, axis=0)

            vp_best = vp_values[int(np.argmin(misfit_grid_vp_local))]
            vs_best = vs_values[int(np.argmin(misfit_grid_vs_local))]
            ...
            stage_results[dmin] = {
                "total": misfit_layer_total,
                "vp": misfit_layer_vp,
                "vs": misfit_layer_vs,
                "vp_best": float(vp_best),
                "vs_best": float(vs_best),
                "misfit_best": misfit_best,
            }
            stage_scores[dmin] = misfit_best

            logger.info(
                f"{stage_label} | camada {dmin:.0f}-{dmax:.0f} km | "
                f"Vp*={vp_best:.2f} km/s, Vs*={vs_best:.2f} km/s, misfit_total_min={misfit_best:.4f}"
            )
        return stage_scores

    # ------------------------------------------------------------------
    # 4) Etapa 1: step de 60 km em toda a faixa
    # ------------------------------------------------------------------
    stage1_scores = _evaluate_and_store(stage1_depths, "Etapa 1/3 (step 60 km, grid 7x7)")

    stage1_best_depths = np.asarray(
        sorted(stage1_scores, key=stage1_scores.get)[: min(5, len(stage1_scores))],
        dtype=float,
    )

    # ------------------------------------------------------------------
    # 5) Etapa 2: step de 40 km ao redor dos 5 melhores da etapa 1
    # ------------------------------------------------------------------
    stage2_depths = _depth_refine_candidates(stage1_best_depths, 40.0, full_depth_windows, half_width=3)
    stage2_scores = _evaluate_and_store(stage2_depths, "Etapa 2/3 (step 40 km, grid 7x7)")

    stage2_best_depths = np.asarray(
        sorted(stage2_scores, key=stage2_scores.get)[: min(3, len(stage2_scores))],
        dtype=float,
    )

    # ------------------------------------------------------------------
    # 6) Etapa 3: step de 20 km ao redor dos 3 melhores da etapa 2
    # ------------------------------------------------------------------
    stage3_depths = _depth_refine_candidates(stage2_best_depths, 20.0, full_depth_windows, half_width=3)
    _ = _evaluate_and_store(stage3_depths, "Etapa 3/3 (step 20 km, grid 7x7)")

    # ------------------------------------------------------------------
    # 7) Monta cubos finais em toda a malha de 20 km via interpolação em profundidade
    # ------------------------------------------------------------------
    sampled_depths = np.asarray(sorted(stage_results.keys()), dtype=float)

    sampled_cube_total = np.stack([stage_results[d]["total"] for d in sampled_depths], axis=0)
    sampled_cube_vp = np.stack([stage_results[d]["vp"] for d in sampled_depths], axis=0)
    sampled_cube_vs = np.stack([stage_results[d]["vs"] for d in sampled_depths], axis=0)

    depth_windows = full_depth_windows
    misfit_cube_total = _interpolate_cube_along_depth(sampled_depths, sampled_cube_total, depth_windows)
    misfit_cube_vp = _interpolate_cube_along_depth(sampled_depths, sampled_cube_vp, depth_windows)
    misfit_cube_vs = _interpolate_cube_along_depth(sampled_depths, sampled_cube_vs, depth_windows)

    # ------------------------------------------------------------------
    # 8) Saídas finais por camada (na malha regular de 20 km)
    # ------------------------------------------------------------------
    vp_best_list = np.zeros(len(depth_windows), dtype=float)
    vs_best_list = np.zeros(len(depth_windows), dtype=float)
    misfit_best_list = np.zeros(len(depth_windows), dtype=float)
    ...
    for i, dmin in enumerate(depth_windows):
        misfit_layer_total = misfit_cube_total[i, :, :]
        misfit_layer_vp = misfit_cube_vp[i, :, :]
        misfit_layer_vs = misfit_cube_vs[i, :, :]

        misfit_grid_vp[i, :] = np.min(misfit_layer_vp, axis=1)
        misfit_grid_vs[i, :] = np.min(misfit_layer_vs, axis=0)

        vp_best_list[i] = vp_values[int(np.argmin(misfit_grid_vp[i, :]))]
        vs_best_list[i] = vs_values[int(np.argmin(misfit_grid_vs[i, :]))]
        idx_min = np.unravel_index(np.argmin(misfit_layer_total), misfit_layer_total.shape)
        misfit_best_list[i] = float(misfit_layer_total[idx_min])
    ...
    # ------------------------------------------------------------------
    # 6.5) CHECKPOINT (salva tudo ANTES de plotar) — para não perder resultados
    # ------------------------------------------------------------------
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    ckpt_path = os.path.join(OUTPUT_DIR, "three_stage_checkpoint_17.npz")
    try:
        np.savez(
            ckpt_path,
            run_timestamp=np.array([__import__("datetime").datetime.now().isoformat()]),
            # catálogo
            event_ids=np.asarray(events["event_id"].astype(str).values),
            # grade e resultados por camada
            depth_windows=np.asarray(depth_windows, float),
            step_km=np.asarray([float(step)], float),
            depth_center_km=np.asarray(depths_center, float),
            vp_values=np.asarray(vp_values, float),
            vs_values=np.asarray(vs_values, float),
            vp_best_list=np.asarray(vp_best_list, float),
            vs_best_list=np.asarray(vs_best_list, float),
            misfit_total_min=np.asarray(misfit_best_list, float),
            # mapas e cubos de misfit
            misfit_grid_vp=np.asarray(misfit_grid_vp, float),
            misfit_grid_vs=np.asarray(misfit_grid_vs, float),
            misfit_cube_vp=np.asarray(misfit_cube_vp, float),
            misfit_cube_vs=np.asarray(misfit_cube_vs, float),
            misfit_cube_total=np.asarray(misfit_cube_total, float),
            # melhor ajuste global (P/S/TOT)
            bestP_fun=np.asarray([float(best_P.get("fun", np.nan))]),
            bestP_vp0=np.asarray([float(best_P.get("vp0", np.nan))]),
            bestP_vs0=np.asarray([float(best_P.get("vs0", np.nan))]),
            bestP_dvp=np.asarray(best_P.get("dvp", []), float),
            bestP_dvs=np.asarray(best_P.get("dvs", []), float),
            bestP_d=np.asarray(best_P.get("d", []), float),
            bestP_w=np.asarray(best_P.get("w", []), float),

            bestS_fun=np.asarray([float(best_S.get("fun", np.nan))]),
            bestS_vp0=np.asarray([float(best_S.get("vp0", np.nan))]),
            bestS_vs0=np.asarray([float(best_S.get("vs0", np.nan))]),
            bestS_dvp=np.asarray(best_S.get("dvp", []), float),
            bestS_dvs=np.asarray(best_S.get("dvs", []), float),
            bestS_d=np.asarray(best_S.get("d", []), float),
            bestS_w=np.asarray(best_S.get("w", []), float),

            bestTOT_fun=np.asarray([float(best_TOT.get("fun", np.nan))]),
            bestTOT_vp0=np.asarray([float(best_TOT.get("vp0", np.nan))]),
            bestTOT_vs0=np.asarray([float(best_TOT.get("vs0", np.nan))]),
            bestTOT_dvp=np.asarray(best_TOT.get("dvp", []), float),
            bestTOT_dvs=np.asarray(best_TOT.get("dvs", []), float),
            bestTOT_d=np.asarray(best_TOT.get("d", []), float),
            bestTOT_w=np.asarray(best_TOT.get("w", []), float),

            # curvas do modelo completo
            zfine_P=np.asarray(curves_P[0], float),
            vp_fine_P=np.asarray(curves_P[1], float),
            vs_fine_P=np.asarray(curves_P[2], float),

            zfine_S=np.asarray(curves_S[0], float),
            vp_fine_S=np.asarray(curves_S[1], float),
            vs_fine_S=np.asarray(curves_S[2], float),

            zfine_TOT=np.asarray(curves_TOT[0], float),
            vp_fine_TOT=np.asarray(curves_TOT[1], float),
            vs_fine_TOT=np.asarray(curves_TOT[2], float),

            # metadados úteis
            N_DISC=np.asarray([int(N_DISC)]),
            DEPTH_START_KM=np.asarray([float(DEPTH_START_KM)]),
            DEPTH_END_KM=np.asarray([float(DEPTH_END_KM)]),
        )
        logger.info(f"[checkpoint] Resultados completos salvos em {ckpt_path}")
    except Exception as e:
        logger.error(f"[checkpoint] Falha ao salvar checkpoint: {e}")
    ...
    try:
        ...
        logger.exception("Falha ao plotar Fig. 3 heatmaps; seguindo (checkpoint/CSV ainda serão salvos).")
    ...
    try:
        ...
        logger.exception("Falha ao plotar figura de transição; seguindo (checkpoint/CSV ainda serão salvos).")
    ...
    disc_txt = os.path.join(OUTPUT_DIR, "mtz_discontinuities_bestTOT_17.txt")
    ...
    out_csv = os.path.join(OUTPUT_DIR, "layered_inversion_results_17.csv")
