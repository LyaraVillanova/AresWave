...
# ---------------------------------------------------------------------------
def main():
    logger.info("=== Iniciando inversão camada a camada (busca em duas etapas) ===")
    ...
    # Busca em duas etapas: grid grosso em todas as camadas + refinamento local
    n_vp_coarse = 7
    n_vs_coarse = 7
    top_k_refine = 5
    vp_values_coarse = np.linspace(vp_values.min(), vp_values.max(), n_vp_coarse)
    vs_values_coarse = np.linspace(vs_values.min(), vs_values.max(), n_vs_coarse)
    ...
    # ------------------------------------------------------------------
    # 5) Etapa 1: grid grosso em todas as camadas
    # ------------------------------------------------------------------
    coarse_best = np.zeros(len(depth_windows), dtype=float)

    for i, dmin in enumerate(depth_windows):
        dmax = dmin + step
        logger.info(f"--- Etapa 1/2 | camada {dmin:.0f}-{dmax:.0f} km (grid grosso) ---")

        m_tot_c, m_vp_c, m_vs_c = _evaluate_layer_grid(events, dmin, dmax, vp_values_coarse, vs_values_coarse)

        misfit_layer_total = _resample_layer_grid(vp_values_coarse, vs_values_coarse, m_tot_c, vp_values, vs_values)
        misfit_layer_vp = _resample_layer_grid(vp_values_coarse, vs_values_coarse, m_vp_c, vp_values, vs_values)
        misfit_layer_vs = _resample_layer_grid(vp_values_coarse, vs_values_coarse, m_vs_c, vp_values, vs_values)
        ...
        vp_best_list[i] = vp_best
        vs_best_list[i] = vs_best
        misfit_best_list[i] = misfit_best
        coarse_best[i] = misfit_best

        logger.info(
            f"Etapa 1/2 | camada {dmin:.0f}-{dmax:.0f} km | "
            f"Vp*={vp_best:.2f} km/s, Vs*={vs_best:.2f} km/s, misfit_total_min={misfit_best:.4f}"
        )

    # ------------------------------------------------------------------
    # 6) Etapa 2: refinamento local nas melhores camadas
    # ------------------------------------------------------------------
    k = min(top_k_refine, len(depth_windows))
    refine_idx = np.argsort(coarse_best)[:k]

    for i in refine_idx:
        dmin = depth_windows[i]
        dmax = dmin + step
        logger.info(f"--- Etapa 2/2 | camada {dmin:.0f}-{dmax:.0f} km (grid fino) ---")

        misfit_layer_total, misfit_layer_vp, misfit_layer_vs = _evaluate_layer_grid(events, dmin, dmax, vp_values, vs_values)
        ...
        logger.info(
            f"Etapa 2/2 | camada {dmin:.0f}-{dmax:.0f} km | "
            f"Vp*={vp_best:.2f} km/s, Vs*={vs_best:.2f} km/s, misfit_total_min={misfit_best:.4f}"
        )
