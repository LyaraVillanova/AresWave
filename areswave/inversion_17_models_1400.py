...
OUTPUT_DIR = "/home/lyara/areswave/outputs_modelsdir17/"
FIG_DIR = "/home/lyara/areswave/figs_modelsdir17/"
FS = 20.0
DT = 1.0 / FS
MAX_SHIFT_S = 5.0
...
P_WINDOW = (-5.0, 5.0)
S_WINDOW = (-5.0, 20.0)
S_WINDOW_EXTENDED_EVENTS = {"S0133a", "S0152a", "S0167a", "S0167b", "S0185a", "S0226b", "S0234c", "S0254b", "S0345a", "S0395a", 
                            "S0421b", "S0976a", "S1000a", "S1094b", "S1102a", "S1153a", "S1415a"}
S_WINDOW_EXTENDED = (-5.0, 10.0)
...
def load_observed_waveforms(event_id: str) -> Optional[Stream]:
    ...
    event_token = re.sub(r"\s+", "", event_id)
    ...
    if len(st) == 0:
        logger.warning(f"[{event_id}] Nenhum SAC encontrado em {search_dir}")
        return None
...
def plot_fig3_ab(suite: Sequence[SuiteModel], results_df: pd.DataFrame, output_dir: str) -> Tuple[str, str]:
    ...
    out_a = os.path.join(output_dir, "fig3A_vp_profiles_pmisfit.png")
    plt.tight_layout()
    ...
    out_b = os.path.join(output_dir, "fig3B_vs_profiles_smisfit.png")
    plt.tight_layout()
...
def plot_fig3_cde(results_df: pd.DataFrame, output_dir: str) -> str:
    ...
    out = os.path.join(output_dir, "fig3CDE_misfit_vs_depth.png")
    plt.tight_layout()
...
def plot_mtz4_scatter(results_df: pd.DataFrame, output_dir: str) -> str:
    ...
    out = os.path.join(output_dir, "fig_mtz4_scatter_misfit_vs_depth.png")
    plt.tight_layout()
    plt.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
def plot_mtz4_paperlike(suite: Sequence[SuiteModel], results_df: pd.DataFrame, output_dir: str) -> str:
    ...
    out = os.path.join(output_dir, "vp_vs_paperlike_mtz4_bestmodel.png")
    plt.tight_layout()
    plt.savefig(out, dpi=300)
    plt.close(fig)
    return out
...
def main(models_dir: str = MODELS_DIR):
    ...
    out_csv = os.path.join(OUTPUT_DIR, "pnas_equivalent_suite_results.csv")
    ...
    summary_txt = os.path.join(OUTPUT_DIR, "best_model_summary.txt")
