...
event_id = 'S0185a_mqs2019kxjd_Geophysical_model298_nd'
...
# Load Geophysical_model298 directly from .nd, using the same conversion workflow
# used in inversion_17_models_1400.py. This avoids editing dsmpy/seismicmodel_Mars.py
# and avoids the "maxnzone - nzone" error from making the full .nd a standalone 117-zone model.
import sys
import importlib.util

PROJECT_DIR = "/home/lyara/areswave"
ARES_SRC_DIR = os.path.join(PROJECT_DIR, "areswave")
INV_SCRIPT_PATH = os.path.join(ARES_SRC_DIR, "inversion_17_models_1400.py")
ND_MODEL_PATH = os.path.join(PROJECT_DIR, "models", "Geophysical_model298.nd")

if ARES_SRC_DIR not in sys.path:
    sys.path.insert(0, ARES_SRC_DIR)

spec = importlib.util.spec_from_file_location("inv17models_runtime", INV_SCRIPT_PATH)
inv17 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(inv17)

base_model = inv17.SeismicModel.test2()
suite_model = inv17.load_suite_model_from_nd(ND_MODEL_PATH, base_model)
seismic_model = inv17.build_full_model_from_suite(base_model, suite_model)

print(f"Loaded ND model: {suite_model.model_id}")
print(f"DSMpy model id: {seismic_model.get_model_id()}")
print(f"DSMpy nzone: {seismic_model._nzone}")
sampling_hz = 20
nspc = 256
tlen = 400
